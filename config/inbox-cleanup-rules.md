...
- **Known gap, not yet resolved**: several other WRKPOD invoices between
  ...
  actioned.
- **Exception — payment-made / remittance-advice notices** (added
  2026-09-10, confirmed with Shaye): some Xero emails matching this rule's
  subject text are not "you have a new bill" invoice notifications at
  all, but remittance advice confirming a payment *already made* by a
  customer/debtor (body reads "Here's your remittance advice for payment
  of AUD ..."). These don't belong at Dext (which processes bills owed,
  not payments received). Confirmed with Shaye 2026-09-10: for these,
  **forward to `tracey@jog.com.au` instead of `tvcollective@dext.cc`**,
  then archive (same idempotency check — skip the forward if already
  forwarded). Two separate runs had flagged the same email as an
  ambiguous question before this was confirmed (see thread
  `1a087cc5bcf63683`, 2026-09-09, and thread `1a0891179c23debe`,
  2026-09-10) — both resolved by this answer.
  - **Validated**: 2026-09-10 — the one open case at the time, "[Stackers
    Finance] Payment has been made by Sina Solutions Pty Ltd for The
    Verge Collective Pty Ltd for AUD 771.56" (thread `1a0863a0fa463f81`),
    was forwarded to `tracey@jog.com.au` and archived live — confirmed
